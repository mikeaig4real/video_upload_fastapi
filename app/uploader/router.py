...
async def get_params(
    upload: UploadType, session: RequireSession, current_user: RequireCurrentUser
):
    ...
    params = await uploader_crud.generate_params(asset_id=asset_id, resource_type="video")
    await upload_crud.create(
        data=UploadCreate(asset_id=asset_id, user_id=current_user.id, upload_status=UPLOAD_STATUS_ENUM.PROCESSING, **upload.model_dump(exclude={"id", "folder"})),
...
async def get_resource(
    ...
    resource = await uploader_crud.get_resource(asset_id=asset_id, resource_type="video")
